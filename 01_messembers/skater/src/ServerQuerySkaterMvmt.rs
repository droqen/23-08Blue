...
pub fn setup() -> Listener {
    query((translation(), skater_target(), skater_fwd(), skater_pace(), linear_velocity())).each_frame(|mice|for(mouse,(pos,cheese,fwd,pace,vel)) in mice{
        ...
        entity::set_component(mouse, skater_pace(), tow(pace,goalpace,PACE_CHANGE_RATE*dt));
        if dist_to_cheese > TURNSPDMULT_FROM_REACH.0 {
            ...
            if rotdist > turnspd * dt {
                entity::set_component(mouse, skater_fwd(), rota.lerp(rotb, turnspd * dt / rotdist) * fwd);
            } else {
...
use crate::embers::skater::components::{skater_target, skater_fwd, skater_pace};