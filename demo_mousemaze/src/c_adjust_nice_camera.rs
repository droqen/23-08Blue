use ambient_api::{prelude::*, core::transform::components::lookat_target};
...
pub fn setup() {
    spawn_query(the_nice_camera()).bind(|cameras|for(camera, _)in cameras{
        ...
        entity::add_component(camera, lookat_target(), vec3(60., 60., 0.));

        // head_relpos: y = move back, z = move up
        entity::add_component(camera, head_relpos(), vec3(0., 40., 40.));
